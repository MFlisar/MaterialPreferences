...
class InputIntViewHolder(
...
) : InputNumberViewHolder<Int, InputIntPreference>(inflater, parent, adapter) {
    ...
    override fun onDialogResultAvailable(
        ...
        preference as InputIntPreference
        if (event is DialogInput.Event.Result) {
            update(event.input.toIntOrNull() ?: preference.defaultValue, preference)
        }
    }
}