...
class InputDoubleViewHolder(
    inflater: LayoutInflater,
    parent: ViewGroup,
    adapter: PreferenceAdapter
) : InputNumberViewHolder<Double, InputDoublePreference>(inflater, parent, adapter) {
    ...
    override fun onDialogResultAvailable(
        ...
        preference as InputDoublePreference
        if (event is DialogInput.Event.Result) {
            update(event.input.toDoubleOrNull() ?: preference.defaultValue, preference)
        }
    }
}